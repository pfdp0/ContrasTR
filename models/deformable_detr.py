...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1],
               'pred_embedds': hs[-1], 'track_embeddings': outputs_tracking_embeddings[-1]}
        if self.aux_loss:
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord, hs, outputs_tracking_embeddings):
        ...
        return [{'pred_logits': a, 'pred_boxes': b, 'pred_embedds': c, 'track_embeddings': d}
                for a, b, c, d in zip(outputs_class[:-1], outputs_coord[:-1], hs[:-1], outputs_tracking_embeddings[:-1])]


class EmbeddingsContrastiveLoss(nn.Module):
    def __init__(self, tau: float = 0.1):
        super(EmbeddingsContrastiveLoss, self).__init__()
    ...
    def forward(self, embeddings: Tensor, labels: Tensor):
        ...
        labels: [num_embeddings, ]
        """
        assert embeddings.shape[0] == labels.shape[0], "Each embeddings should have a label"

        num_embeddings = embeddings.shape[0]

        positive_pairs = torch.eq(labels[None, :], labels[:, None])
        ...
        # is NOT the same as for tracking !!!
        # (here, we don't have to handle the case with more than one positive pair per id)
        logprob = - F.log_softmax(similarities, dim=1)  # [num_embeddings, num_embeddings]

        logprob[torch.logical_not(positive_pairs)] = 0.

        loss = logprob.sum(dim=1) / torch.clamp(positive_pairs.sum(dim=1), min=1.)  # [num_embeddings, 1]
        loss_value = self.tau * loss.sum() / torch.clamp((positive_pairs.sum(dim=1) != 0).sum(), min=1.)
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, losses, focal_alpha=0.25,
                 use_contrastive_loss: bool = False, temperature=0.07):
        ...
        if self.use_contrastive_loss:
            # contrastive loss
            self.contrastive_loss = EmbeddingsContrastiveLoss(tau=temperature)
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes):
        ...
        card_pred = (torch.amax(pred_logits.sigmoid(), dim=2) >= 0.5).sum(dim=1)
    ...
    def loss_contrastive(self, outputs, targets, indices, num_boxes):
        ...
        # collect predicted embeddings
        embeddings = [outputs['track_embeddings'][b, index_i, :] for b, (index_i, index_j) in enumerate(indices)]
        embeddings = torch.cat(embeddings)

        # retrieve ground truth ids (i.e. positives are augmented objects from the same image)
        embeddings_ids = torch.tensor([
            batch_targets['image_id'].item() * 1_000_000 + instance_id.item()
        ...
        if is_dist_avail_and_initialized():
            ...
            embeddings_list = [torch.empty((max_num_embed, embed_dim), device="cuda", dtype=float_type) for _ in
                               num_embed_list]
            if local_num_embed != max_num_embed:
                padding = torch.empty(size=(max_num_embed - local_num_embed, embed_dim), dtype=float_type,
                                      device="cuda")
            ...
            embeddings_ids_list = [torch.empty(max_num_embed, device="cuda", dtype=torch.long) for _ in num_embed_list]
            if local_num_embed != max_num_embed:
                padding = torch.empty(size=(max_num_embed - local_num_embed,), dtype=torch.long, device="cuda")
...
def build(args):
    if args.dataset_file == 'kitti':
        ...
        num_classes = 2     # CrowdHuman has 1 class, class 0 will be unused
    # num_classes = 20 if args.dataset_file != 'coco' else 91
    elif args.dataset_file == "coco_panoptic":
        num_classes = 250
    else:
        ValueError(f"The number of classes for dataset {args.dataset_file} is unknown")
    ...
    model = DeformableDETR(
        ...
        use_contrastive_loss=args.contrastive_pretraining,
    )
    if args.masks:
    ...
    criterion = SetCriterion(num_classes, matcher, weight_dict, losses, focal_alpha=args.focal_alpha,
                             use_contrastive_loss=args.contrastive_pretraining, temperature=args.cont_loss_temp)
    criterion.to(device)
