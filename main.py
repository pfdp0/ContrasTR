...
def get_args_parser():
    ...
    parser.add_argument('--contrastive_pretraining', action='store_true',
                        help="Enables contrastive pre-training (doesn't require tracking annotations)")
    ...
    parser.add_argument('--cont_loss_coef', default=1, type=float,
                        help="Contrastive loss coefficient (only used if contrastive_pretraining is used)")
...
def main(args):
    ...
    if args.contrastive_pretraining:
        print(f"Using contrastive pretraining with coef {args.cont_loss_coef} and temperature {args.cont_loss_temp}")
        data_loader_train = DataLoader(dataset_train, batch_sampler=batch_sampler_train,
                                       collate_fn=utils.collate_fn_contrast, num_workers=args.num_workers,
                                       pin_memory=True)
    else:
        data_loader_train = DataLoader(dataset_train, batch_sampler=batch_sampler_train,
    ...
    if args.resume:
        ...
        if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint and not args.from_pretrained:
            import copy
    ...
    if args.eval:
        test_loss_stats, test_metrics_stats, coco_evaluator = evaluate(model, criterion, postprocessors,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_loss_stats, test_metrics_stats, coco_evaluator = evaluate(
            model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir, args
        )
        if args.distributed:
        ...
        if wandb.run is not None:
            test_detection_precisions = {k: v for k, v in
                                         zip(utils.DETECTION_METRICS_NAMES[:6], test_metrics_stats['coco_eval_bbox'][:6])}
            test_detection_recall = {k: v for k, v in
                                     zip(utils.DETECTION_METRICS_NAMES[6:], test_metrics_stats['coco_eval_bbox'][6:])}

            log_dict = {
                "Loss/val": test_loss_stats["loss"],
                "Loss_dict/val": test_loss_stats,
        ...
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     **{f'test_{k}': v for k, v in test_loss_stats.items()},
                     **{f'test_{k}': v for k, v in test_metrics_stats.items()},
                     'epoch': epoch,
