...
def collate_fn_contrast(batch):
    batch = list(zip(*batch))
    batch[0] = nested_tensor_from_tensor_list([img for imgs in batch[0] for img in imgs])
    batch[1] = [tgt for tgts in batch[1] for tgt in tgts]
    return tuple(batch)
...
def init_distributed_mode(args):
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        torch.set_num_threads(8)  # FIXME: find a better way to configure this
