...
class CocoDetection(TvCocoDetection):
    def __init__(self, img_folder, ann_file, transforms, return_masks, cache_mode=False, local_rank=0, local_size=1,
                 n_views: int = 1, prepare_fn: Optional[Callable] = None):
        super(CocoDetection, self).__init__(img_folder, ann_file,
                                            cache_mode=cache_mode, local_rank=local_rank, local_size=local_size)
        self._transforms = transforms
        assert n_views in [1, 2], f"Currently only support 1 or 2 views, got {n_views}"
        self.n_views = n_views
        if prepare_fn is not None:
            self.prepare = prepare_fn(return_masks)
        else:
            print("Using default prepare function for CocoDetection")
...
def build(image_set, args):
    ...
    n_views = 2 if (image_set == "train" and args.contrastive_pretraining) else 1
